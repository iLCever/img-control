...
- `img` 根路径公开展示图片网格，右上角登录按钮跳转到独立管理域名
...
- `img.moxiao.ggff.net/` 无需登录即可浏览、搜索、预览和复制图片链接。
- `img.moxiao.ggff.net/YYYY/MM/UUID.扩展名` 从 KV 返回图片。
- 图片域名只开放列表与统计 GET API；登录、上传、删除等管理 API 会返回 404。

## 部署后测试

1. 打开 `https://img.moxiao.ggff.net/`，确认无需登录即可显示公开图片网格。
...
8. 退出后 `/api/images` 和 `/api/stats` 仍可只读访问，`/api/upload` 与 `DELETE /api/images` 应返回 401。
9. 手机端确认网格为两列。
...
- 上传、删除、退出等管理 API 在 Pages Functions 中验证 Session；图片列表和统计为公开只读接口。
- 上传同时检查扩展名、Content-Type 和文件头，服务端生成 `YYYY/MM/UUID.扩展名` Key。
- 图片二进制保存为 KV value，大小、类型、原始文件名和上传时间保存在 KV metadata。
- 图片域名中间件只允许前端静态资源、只读目录 API 和合法图片路径，禁止登录及写入 API。
