...
- 图片与入口页：`https://img.moxiao.ggff.net`
...
- `img` 根路径内置管理员入口页，不需要额外上传 HTML 或 URL Rewrite Rule
...
- `img-admin.moxiao.ggff.net` 提供管理后台和 API。
- `img.moxiao.ggff.net/` 提供管理员入口页。
- `img.moxiao.ggff.net/YYYY/MM/UUID.扩展名` 从 KV 返回图片。
- 图片域名上的管理 API 和其他路径会返回 404。

## 部署后测试

1. 打开 `https://img.moxiao.ggff.net/`，确认显示入口页。
2. 点击“管理员登录”，确认跳转到 `https://img-admin.moxiao.ggff.net/login`。
...
8. 退出后直接调用 `/api/images`、`/api/upload` 或 `/api/stats`，应返回 401。
9. 手机端确认网格为两列。
...
| GET | `/api/images?cursor=&limit=30&search=` | 列表、搜索与分页 |
| POST | `/api/upload` | multipart 字段 `file`，每个请求一张 |
| DELETE | `/api/images` | JSON：`{ "keys": ["..."] }`，最多 100 个 |
| GET | `/api/stats` | 遍历 KV 元数据计算数量和大小 |
